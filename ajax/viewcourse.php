...
if (is_dir($CFG->dirroot.'/mod/customlabel')) {
    ...
    $authors = customlabel_get_authors($course->id);
    $template->authors = $authors->authors;
    $template->contributors = $authors->contributors;

    $coursemetadata = customlabel_get_coursedata($course->id);
    $template->metadata = $coursemetadata;

    $courseclassifiers = customlabel_get_classifiers($course->id);
    $template->categories = $courseclassifiers;
